	...
	for (;;){
		...
		while(terminar==0)
		{
			...
			if(codigo==0) 	
				...
				char agregar[80];
				if(numero == 0){
					strcpy(agregar, "INSERT INTO jugador (username, password, partidas, ganadas) VALUES ('");					
				...
				strcpy(consulta, "SELECT jugador.ganadas FROM jugador WHERE jugador.username ='");
				...
				strcpy(consulta, "SELECT partida.idpartida FROM partida WHERE partida.duracion = (SELECT MAX(partida.duracion) FROM partida, jugador, relacion WHERE partida.idpartida = relacion.idpartida AND relacion.username = jugador.username AND jugador.username = '");
				strcat(consulta, nombre);
				strcat(consulta, "');");
				...
				strcpy(consulta, " SELECT partida.idpartida FROM partida, jugador, relacion WHERE  jugador.username = '");
				strcat(consulta, nombre1);
				strcat(consulta, "' AND jugador.username = relacion.username AND relacion.idpartida IN (SELECT partida.idpartida FROM jugador, partida, relacion WHERE partida.idpartida = relacion.idpartida AND relacion.username = jugador.username AND jugador.username= '");
